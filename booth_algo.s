...
AQq
	...
	MOV R9, R7; storing duplicate AQq
	;extract A, Q, q from above AQq
	; getting MSB of AQq, i.e. q
	and R3, R7, R1
	MOV R9, R9, lsr 1; 
	;getting Q from AQq
	MOV R5, 1; we need value 1 in register for next step
	MOV R5, R5 , lsl R10; 1 << bit; 10000
	sub R5, R5, 1; (1 << bits - 1);  1111
	and R2, R9, R5; R9 & (1 << bits - 1)
	MOV R9, R9, lsr R10
	;getting A from AQq
	MOV R1, R9
	;MOV R8, R8, asr 1
...
ylabel
	...
	MOV R9, R9 , lsl R6; 1 << bits 10000
	sub R9, R9, 1; (1 << bits - 1)  1111
	and R4, R4, R9; M & (1 << bits - 1)
	add R4, R4, 1
	add R1, R1, R4
	BX LR
; A = A + M
